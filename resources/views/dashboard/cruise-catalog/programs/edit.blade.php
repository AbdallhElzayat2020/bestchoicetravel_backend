...
@section('title', 'Edit cruise program')

@section('content')
    @php
        $dayRows = old('days');
        if ($dayRows === null) {
            $dayRows = $program->days
                ->map(
                    fn ($d) => [
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.cruise-catalog.programs.update', $program) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    ...
                    <select name="cruise_catalog_category_id" class="form-select" required>
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}"
                                @selected(old('cruise_catalog_category_id', $program->cruise_catalog_category_id) == $cat->id)>{{ $cat->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" name="duration_days" class="form-control" min="1" max="365"
                            value="{{ old('duration_days', $program->duration_days) }}" required>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Status</label>
                        <select name="status" class="form-select">
                            <option value="active" @selected(old('status', $program->status) === 'active')>Active</option>
                            <option value="inactive" @selected(old('status', $program->status) === 'inactive')>Inactive
                            </option>
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
