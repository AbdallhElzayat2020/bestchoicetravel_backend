...
(() => {
    const hidePreloader = () => {
        ...
        if (
            !preloader ||
            preloader.classList.contains("site-preloader--hidden")
        )
            return;
...
if (document.querySelector(".review-badges-carousel")) {
    // eslint-disable-next-line no-unused-vars
    // Show 3 logos in view and animate only the logo element (marquee-like)
    const reviewBadgesSwiper = new Swiper(".review-badges-carousel", {
        ...
        freeModeMomentum: false,
            // medium speed with zero delay for a smooth continuous infinite scroll
            speed: 5000,
        autoplay: {
            delay: 0,
            disableOnInteraction: false,
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const tourLightboxBackdrop = document.getElementById(
        "tourLightboxBackdrop",
    );
    ...
    document.addEventListener("keydown", (e) => {
        if (
            e.key === "Escape" &&
            tourLightbox &&
            tourLightbox.classList.contains("open")
        ) {
            closeTourLightbox();
        }
    });
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const cards = Array.from(
        document.querySelectorAll(".tours-card[data-tour]"),
    );
    ...
    const chips = Array.from(
        document.querySelectorAll(".tours-filter-chip[data-tour-type]"),
    );
    ...
    const hasFilterControls =
        chips.length > 0 || searchInput || sortSelect || durationSelect;
    ...
    const setActiveChip = (type) => {
        ...
        chips.forEach((chip) =>
            chip.classList.toggle(
                "active",
                chip.dataset.tourType === activeType,
            ),
        );
    };
    ...
    const matches = (card, filters) => {
        ...
        if (filters.durationRange) {
            if (
                !(
                    days >= filters.durationRange.min &&
                    days <= filters.durationRange.max
                )
            )
                return false;
        }
    ...
    const applyVisibility = (filters) => {
        ...
        sections.forEach((section) => {
            const sectionCards = section.querySelectorAll(
                ".tours-card[data-tour]",
            );
            const anyVisible = Array.from(sectionCards).some(
                (c) => !c.classList.contains("tours-card--hidden"),
            );
        ...
        grids.forEach((grid) => {
            const visibleCards = Array.from(
                grid.querySelectorAll(
                    ".tours-card[data-tour]:not(.tours-card--hidden)",
                ),
            );
            const hiddenCards = Array.from(
                grid.querySelectorAll(
                    ".tours-card[data-tour].tours-card--hidden",
                ),
            );
...
document.querySelectorAll(".explore-btn").forEach((button) => {
    button.addEventListener("click", function () {
        const packageTitle =
            this.closest(".package-card").querySelector(
                ".package-title",
            ).textContent;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const fallbackImages = {
        ...
        pyramids:
            "https://images.unsplash.com/photo-1539650116574-75c0c6d73a6e?w=800&h=400&fit=crop",
    ...
    images.forEach((img) => {
        ...
        if (img.complete && img.naturalHeight !== 0) {
            ...
            img.addEventListener("error", function handleError() {
                console.warn(
                    "Image failed to load, trying fallback:",
                    this.src,
                );
                this.classList.remove("img-loading");
    ...
    const DESERT_VIDEO_URL =
        "https://www.youtube.com/embed/2ElPeQQMaXw?si=90ZowOqIhtOM-fC-&autoplay=1";
    ...
    document.addEventListener("keydown", (e) => {
        if (
            e.key === "Escape" &&
            desertVideoModal &&
            desertVideoModal.classList.contains("open")
        ) {
            closeDesertVideo();
        }
    });
    ...
    if (
        ...
        const setAnnouncementDuration = () => {
            ...
            const seconds = Math.min(
                240,
                Math.max(40, distancePx / pxPerSecond),
            );
